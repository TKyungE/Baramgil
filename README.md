...
지금 서 있는 길목의 풍속·돌풍·풍향과, 진행 방향 앞 길목의 바람, 그리고 6시간 뒤까지 15분 단위 예측을 지도 위에 보여주는 웹앱(PWA).
...
- **예측 타임라인** (아래 막대 그래프): 지금부터 6시간 뒤까지 15분 간격 25칸. 막대 높이 = 그 시각에 내 길목이 받을 풍속, 색 = 단계 → 언제 세지는지 한눈에. 막대를 탭하거나 끌면(키보드 ←→도 됨) 지도 전체가 그 시각 기준으로 바뀐다: 길 색·흐름, 위 카드(‘예측’ 표시 + `+45분 · 14:52`), 다음 길목, 장소 시트의 바람, 경로의 바람 프로필·대안 순위까지. [지금으로]로 복귀. 1분마다 ‘+45분’의 실제 시각을 다시 계산하고, 바람 갱신(10분)마다 새 예보로 바꾼다. (Windy의 하단 타임라인·날씨앱의 시간별 그래프와 같은 자리·같은 조작.)
...
- **도보 길찾기**: Valhalla 공개 서버(키 없음, 한국어 안내). 경로를 길목 단계 색으로 칠하고 단계별 거리 막대를 보여줌. 대안 경로는 '센 바람 점수'(강풍 이상을 걷는 거리; 위험 3배, 주의는 0.15배 — 평균이 아니라 길이 합)로 순위 → **센 바람 피하는 길 / 가장 빠른 길** 칩(각각 소요 시간과 강풍 이상 거리 표시). 센 바람 피하는 길이 빠른 길보다 1.4배 넘게 오래 걸리면 기본 선택은 빠른 길(`js/route.js` `DETOUR_MAX`). 안내 중엔 아래 카드가 '경로 기준 다음 길목'으로 바뀜. (자전거 앱 Headwind·BikeWind가 바람을 고려해 경로를 고르는 것과 같은 발상을 보행자·길목 단위로.)
...
- 길목 조회는 격자 색인(약 110 m 셀)으로 주변만 검사. 방향 원뿔은 나침반 이벤트를 전부 받아 1€ 필터(Casiez 외 2012)로 걸러 매 프레임 그리고(정지 시 떨림 제거, 회전 시 지연 없음), 계산이 무거운 '다음 길목 탐색'만 3°·250 ms 마다.
- 길·건물은 타일에서 80 m 이상 움직였을 때만 다시 꺼내고, 건물은 본 곳을 기억(중심 2.5 km 밖은 버림).
- 예측 시각을 끌어 바꿀 때 지도 소스(길 기하)는 다시 올리지 않고 색만 feature-state 로 바꾼다(2,400 길목에서 계산 약 9 ms). 입자는 같은 길목 위의 것을 이어 써서 흐름이 끊기지 않고 속도·방향만 바뀜. 끌기 중엔 60 ms에 한 번만 다시 계산.
...
| 배경 바람(지금) | **Open-Meteo 예보 모델값** (실측 아님, 15분~1시간 단위) | 기상청 AWS 1분 관측 연결 (아래) |
| 예측(6시간) | Open-Meteo `minutely_15` 24칸+4. **한국은 15분 원자료가 없어 1시간 모델값을 보간한 값**(원자료는 북미 HRRR·유럽 ICON-D2/AROME만). 15분 단위는 표시 해상도이지 예보 해상도가 아님 | `CONFIG.model = 'kma_seamless'`로 기상청 LDPS(1.5 km, 1시간) 지정 가능(실패 시 자동으로 기본 모델). 더 가면 기상청 초단기예보(6시간, 1시간 간격)를 프록시로 (아래) |
...
2. **기상청 연결** — `js/app.js` `CONFIG.kmaProxyUrl` 에 프록시 주소를 넣으면 우선 사용. 브라우저에서 기상청 API를 직접 못 부르므로(키 노출·CORS) 작은 서버(예: Cloudflare Worker)가 `GET {proxy}?lat=..&lon=..` 요청에 아래 JSON을 `Access-Control-Allow-Origin: *` 로 돌려주면 된다.
   ...
   ```
   `speed` 10분 평균(m/s), `dir` 불어오는 방향(도), `gust` 최대 순간풍속(m/s). `time`은 ISO 문자열(KST) 또는 unixtime.
   예보까지 주려면 `forecast` 배열을 붙이면 된다 — 기상청 **초단기예보**(1시간 간격, 6시간)를 그대로 넣어도 앱이 15분으로 보간한다. 없으면 예보만 Open-Meteo에서 받는다.
   ```json
   { "speed": 4.1, "dir": 315, "gust": 7.2, "time": "2026-09-09T10:42", "source": "기상청 AWS 417",
     "forecast": [ { "time": "2026-09-09T11:00", "speed": 4.5, "dir": 320, "gust": 8.0 }, { "time": "2026-09-09T12:00", "speed": 6.0, "dir": 330, "gust": 10.1 } ] }
   ```
...
css/app.css           스타일
js/geo.js             거리·방위 계산
js/wind.js            배경 바람 현재값 + 15분 예보 (Open-Meteo / 기상청 프록시), 시각 보간
js/streets.js         길·건물 데이터 (지도 타일 / Overpass) + 길목 나누기
js/model.js           길목별 풍속·흐름 방향·단계, 래스터 어댑터, 다음 길목 탐색
js/app.js             지도·위치·나침반·카드·예측 타임라인 (설정은 맨 위 CONFIG)
...
- 배경 바람·예보는 10분마다 갱신(한 요청). 길·건물은 화면에 보이는 지도 타일에서 바로 꺼내므로 별도 대기 없음(줌 14 미만에선 골목이 타일에 없어 마지막 결과를 유지).
